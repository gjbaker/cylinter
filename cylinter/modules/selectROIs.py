...
from ..utils import (
    input_check, read_markers, get_filepath, marker_channel_number,
    sort_qc_report, single_channel_pyramid, triangulate_ellipse,
    reorganize_dfcolumns, upscale, ArtifactInfo, artifact_detector_v3
)
...
def selectROIs(data, self, args):
    ...
    try:
        ...
        if reload_report:
            qc_report_sorted = sort_qc_report(
                qc_report, module='selectROIs', order=None
            )
    ...
    if self.samplesForROISelection:
        ...
        if not (set(self.samplesForROISelection).issubset(
            data['Sample'].unique())
        ):
            logger.info(
                'Aborting; one or more samples in "samplesForROISelection" '
                'configuration parameter are not in the dataframe.'
            )
            sys.exit()
        ...
        # points corresponding to centroids of cells classified as artifacts.
        def load_extra_layers():
            ...
            if self.autoArtifactDetection:
                ...
                if self.artifactDetectionMethod == 'MLP':
                    ...
                    # to avoid complication, we implement the most 
                    # straightforward method of registering every abx channel,
                    # even if some may not have artifacts
                    for abx_channel in abx_channels:
                        varname_filename_lst += [
                            (f'{abx_channel}_seeds', 
                             f'{abx_channel}_artifact_seeds.pkl'),
                            (f'{abx_channel}_mask', 
                             f'{abx_channel}_artifact_mask.pkl')
                        ]
                        layer_type += [
                            (f'{abx_channel}_seeds', 'point'),
                            (f'{abx_channel}_mask', 'image') 
                        ]
                        layer_name += [
                            (f'{abx_channel}_seeds', 
                             f'{abx_channel} Artifact Seeds'),
                            (f'{abx_channel}_mask',
                             f'{abx_channel} Artifact Mask')  
                        ]
        ...
        def float_roi_layer_to_top():
            roi_layer_id = viewer.layers.index(
                viewer.layers[layer_name['ROI']]
            )
            top_layer_id = len(viewer.layers)
            viewer.layers.move(roi_layer_id, top_layer_id)
            viewer.layers.select_next()  # ensure manual ROI layer highlighted
        ...
        def add_layers(sample):
            ...
            for varname, layer_data in extra_layers.items():
                ...
                if layer_type[varname] == 'shape':
                    try:
                        shapes = [
                            shape_data[0] for shape_data in layer_data[sample]
                        ]
                        polygons = [
                            np.array(shape_data[-1]) for shape_data in 
                            layer_data[sample]
                        ]
                    except KeyError:
                        shapes = 'polygon'
                    ...
                    if (self.autoArtifactDetection and 
                            self.artifactDetectionMethod == 'classical'): 
                        abx_channel = varname.split('_')[0]
                        
                        try:
                            extra_layers[varname][sample].render_seeds(
                                viewer, global_state.loaded_ims, 
                                layer_name, abx_channel
                            )
                        except:
                            pass 
                    ...
                    elif (self.autoArtifactDetection and 
                          self.artifactDetectionMethod == 'MLP'):
                        try:
                        ...
                        except:  # may want to be explicit about exception here
                            points = None
                            artifact_mask_ = []
                            artifact_proba_ = []
        ...
        def save_shapes(sample):
            ...
            if self.artifactDetectionMethod == 'MLP':
                for i, tupl in enumerate(varname_filename_lst[::-1]):
                    ...
                    if layer_type[varname] == 'shape':
                        for shape_type, roi in zip(layer.shape_type, 
                                                   layer.data):
                            updated_layer_data.append(
                                [shape_type, round(roi.tolist())]
                            )
                        extra_layers[varname][sample] = updated_layer_data
                    elif layer_type[varname] == 'point':
                        updated_layer_data = (
                            layer.data, global_state.artifact_mask, 
                            global_state.artifact_proba
                        )
                        extra_layers[varname][sample] = updated_layer_data
                ...
                for varname, filename in varname_filename_lst:
                    ...
                    if layer_type[varname] == 'shape':
                        ...
                        for shape_type, roi in zip(layer.shape_type, 
                            ...
                            updated_layer_data.append(
                                [shape_type, 
                                 np.round(roi).astype('int').tolist()]
                            )
                        extra_layers[varname][sample] = updated_layer_data
                        save_layer = True
                        ...
                        try:
                            ...
                            if artifact_info is not None:  # layer was deleted 
                    ...
                    if save_layer:
                        if layer_type[varname] == 'shape':
                            ...
                            # empty ROI lists can occur when ROIs are deleted
                            # in Napari, delete key entries in 
                            # extra_layers[varname] with no associated ROIs
                            extra_layers[varname] = {
                                key: value for key, value in 
                                extra_layers[varname].items()
                                if value
                            }

                            qc_report['selectROIs'][
                                layer_name[varname]] = extra_layers[varname]
                            if not bool(extra_layers[varname]):
                                del qc_report[
                                    'selectROIs'][layer_name[varname]]
        ...
        def add_widgets(last_sample, sample):
            ...
            def next_sample(qc_report, last_sample, sample, widgets):
                ...
                try:
                    ...
                    # compute vertices for auto generated artifact ch masks
                    channel_mask_verts = []
                    for varname, filename in varname_filename_lst: 
                    ...
                    if 'Automated ROI Selections (neg.)' not in qc_report[
                     'selectROIs'].keys():
                        qc_report['selectROIs'][
                         'Automated ROI Selections (neg.)'] = {}
                    ...
                    # store automated and manually-defined polygon vertices 
                    # added to qc_report dict by save_shapes function) to 
                    # CyLinter QC report
                    qc_report_sorted = sort_qc_report(
                        qc_report, module='selectROIs', order=None
                    )
            ...
            @magicgui(
                call_button='Enter',
                next_sample={'choices': self.samplesForROISelection,
                             'label': 'Go to Sample'}
            )
            ...
            arbitrary_sample.next_sample.value = sample
            ...
            def label_artifacts_MLP(proba_threshold: float = 0.5):
                ...
                global_state.artifact_mask, class_probas = (
                    artifact_detection_model_MLP(data)
                )
                ...
                points_layer.features.loc[-len(centroids):,
                                          'artifact_class'] = (
            ...
            def on_slider_changed(threshold):
                ...
                points_layer.face_color[:, -1] = np.piecewise(
                    ...
                    [lambda v: 0, lambda v: (v - threshold) / (1 - threshold) * 
                     (1 - 0.3) + 0.3]
                )
            ...
            # overhead of defining these widgets if we never use them
            def generate_widgets_for_classical_method():
                
                channel_selector_dropdown = ComboBox(
                    choices=abx_channels, label='Abx channel'
                )
                sensitivity_spinbox = SpinBox(
                    value=0, label='Sensitivity', min=0, max=255
                )
                ...
                def compute_artifact_mask():
                    ...
                    if abx_channel in artifacts.keys():
                        try:
                            viewer.layers.remove(
                                viewer.layers[
                                    layer_name[f'{abx_channel}_seeds']
                                ]
                            )
                            viewer.layers.remove(
                                viewer.layers[
                                    layer_name[f'{abx_channel}_mask']
                                ]
                            )
                        except:
                            pass
                    ...
                    artifact_info.render(
                        viewer, loaded_ims, layer_name, abx_channel
                    )
                    float_roi_layer_to_top()
                    ...
                    artifact_info.bind_listener_seeds(
                        viewer, global_state, tolerance_spinbox
                    )
                ...
                def update_flood_mask():
                    ...
                    old_fill = flood(
                        im_transformed, 
                        seed_point=tuple(artifacts[abx_channel].seeds[pt_id]), 
                        tolerance=old_tol
                    ) 
                    ...
                    artifacts[abx_channel].seed_layer.features.loc[
                        pt_idx, 'tol'] = new_tol
                
                ###############################################################
                
                widget_lst = [widget_combo_1, widget_combo_2]
                widget_names = ['Automated Artifact Detection', 
                                'Fine-tune Seed Points']
            ...
            if self.autoArtifactDetection:
                if self.artifactDetectionMethod == 'MLP':
                    widgets.append(label_artifacts_MLP)
                    widget_names.append(['Automation Module'])
                elif self.artifactDetectionMethod == 'classical':
                    new_widget_lst, new_widget_names = (
                        generate_widgets_for_classical_method()
                    )
            ...
            for widget, widget_name in zip(widgets, widget_names):
                napari_widgets.append(
                    viewer.window.add_dock_widget(
                        widget=widget, add_vertical_stretch=False, 
                        name=widget_name
                    )
                )
        ...
        def artifact_detection_model_MLP(data):
            ...
            marker_list = [
                'Hoechst0', 'anti_CD3', 'anti_CD45RO', 'Keratin_570', 
                'aSMA_660', 'CD4_488', 'CD45_PE', 'PD1_647', 'CD20_488',
                'CD68_555', 'CD8a_660', 'CD163_488', 'FOXP3_570', 'PDL1_647',
                'Ecad_488', 'Vimentin_555', 'CDX2_647', 'LaminABC_488',
        ...
        extra_layers, layer_type, layer_name, varname_filename_lst = (
            load_extra_layers()
        )
        add_layers(sample)
        ...
        for sample in samples:
            ...
            # might want to keep track if auto artifact detect 
            # used separately in global state
            if self.artifactDetectionMethod == 'MLP':
            ...
            if self.autoArtifactDetection:
                ...
                if autoArtifactDetectionUsed:
                    if self.artifactDetectionMethod == 'MLP':
                        ...
                        inter2 = ~ROI2_mask[ys, xs] & (
                            global_state.artifact_mask != 1) & \
                            (global_state.artifact_proba > 
                             global_state.artifact_detection_threshold)
                    elif self.artifactDetectionMethod == 'classical':
                        masks = []
                        for abx_channel in abx_channels:
                            artifact_info = extra_layers[
                                f'{abx_channel}_mask'].get(sample)
                            if artifact_info is None:
                                continue
                            else:
                                upscaled_mask = upscale(
                ...
                logger.info('Artifact Detection not '
                            f'enabled for sample: {sample}')
            ...
            idxs_to_drop[sample] = list(
                sample_data['CellID'][
                    sample_data['inter1'] | drop_artifact_ids]
            )
        ...
        logger.info(
            'Skipping ROI selection, no samples for ROI '
